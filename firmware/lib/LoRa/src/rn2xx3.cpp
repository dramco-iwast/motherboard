...
void rn2xx3::breakcondition(void){
	...
	// generate break condition on RN2483 RX pin 
	// pin must be low for more than 226 µs (for default data rate of 57600 bps)
	digitalWrite(_rnrx, LOW);
	delayMicroseconds(500);
	digitalWrite(_rnrx, HIGH);
	// change pin function back to sercom (tx)
	pinPeripheral(_rnrx, PIO_SERCOM);

	delayMicroseconds(100);
	// needed for auto-baud detection (triggered by break condition)
...
TX_RETURN_TYPE rn2xx3::txCommand(const String& command, const String& data, bool shouldEncode)
{
  ...
  while(!send_success)
  {
    ...
    switch (determineReceivedDataType(receivedData))
    {
      ...
      {
        DEBUG.println("frame_counter_err_rejoin_needed");
        init();
